...
export async function POST(request: NextRequest) {
  const { email, password, playerId, firstName, lastName, role } = await request.json();

  if (!email || !password) {
    return NextResponse.json({ error: 'Email et mot de passe sont requis' }, { status: 400 });
  }

  // Mode 1: lier un joueur existant → playerId requis
  // Mode 2: créer un nouveau joueur → firstName et lastName requis
  if (!playerId && (!firstName || !lastName)) {
    return NextResponse.json({ error: 'Prénom et nom sont requis pour créer un nouvel utilisateur' }, { status: 400 });
  }
  ...
  // Mode 1: Lier un joueur existant
  if (playerId) {
    ...
    if (accountError) {
      ...
      return NextResponse.json(
        { error: `Erreur lors de la création du profil: ${accountError.message}` },
        { status: 500 }
      );
    }
  ...
  // Mode 2: Créer un nouvel utilisateur + profil joueur
  const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
    email,
    password,
    email_confirm: true,
  ...
  // Créer tournament_players
  const { error: playerError } = await supabaseAdmin.from('tournament_players').insert({
  ...
  if (playerError) {
    await supabaseAdmin.auth.admin.deleteUser(newUserId);
    await supabaseAdmin.from('user_accounts').delete().eq('id', newUserId);
    return NextResponse.json(
      { error: `Erreur lors de la création du profil joueur: ${playerError.message}` },
      { status: 500 }
    );
  }
